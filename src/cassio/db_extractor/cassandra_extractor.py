...
RETRIEVE_ONE_ROW_CQL_TEMPLATE = (
    "SELECT * FROM {keyspace}.{table} WHERE {whereClause} LIMIT 1"
)
...
def _table_primary_key_columns(session, keyspace, table) -> List[str]:
    ...
    return [col.name for col in table.partition_key] + [
...
def _ensure_full_extraction_tuple(tpl, admit_nulls):
    if len(tpl) < 2:
        ...
        raise ValueError(
            "Cannot specify more than (table, column_or_function, admit_nulls, default) in the field_mapper."
        )
...
class CassandraExtractor:
    def __init__(self, session, keyspace, field_mapper, admit_nulls):
        ...
        self.input_parameters = set(
            reduce(
                lambda accum, nw: accum | set(nw), self.primary_key_map.values(), set()
            )
        )
        ...
        def _getter(**kwargs):
            def _retrieve_field(
                ...
                selector = SimpleStatement(
                    RETRIEVE_ONE_ROW_CQL_TEMPLATE.format(
                        keyspace=keyspace,
                        table=_table2,
                        whereClause=" AND ".join(f"{kc} = %s" for kc in _key_columns),
                    )
                )
                values = tuple([_all_pkey_value_map[kc] for kc in _key_columns])
